...
export default {
  ...
  methods:{
    findOne(){
      this.$http.get("http://rap2api.taobao.org/app/mock/297737/user/findOne?id="+this.user.id,).then(res=>{
        console.log(res.data);
        this.user = res.data;
      });
    },
    editUserInfo(){
      this.$http.post("http://rap2api.taobao.org/app/mock/297737/user/update",this.user).then(res=>{
        console.log(res);
        if(res.data.success){
